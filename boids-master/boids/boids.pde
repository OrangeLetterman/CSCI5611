...
void setup () {
  ...
  while(counter < numberofNode){//add node path
    float x = random(width - 10);
    float y = random(height - 10);
    node.add(new Node(new PVector(x, y))); 
    graphNode.add(new Node(new PVector(x, y))); 
    counter++;
    
  }
  node.add(goal);
  for(int i =0; i< numofBoids;i++){//set up a star array
    Astar a = new Astar(node);
    astar.add(a);
...
void draw () {
 ...
 // colorMode(HSB);
  ...
  for(int i = 0; i< node.size();i++){
   ...
   goal.changeColor();
   //set up node path
    for(int i = numofBoids ; i<node.size(); i++){
       for(int j = numofBoids ; j< node.size(); j++){
            Node a = node.get(i);
            Node b = node.get(j);//numofBoids - 1
   ...
   //set up path for boid
   for(int i = 0; i<numofBoids; i++){
    for(int j = numofBoids - 1;j< node.size(); j++){
      Node a = node.get(i);
      Node b = node.get(j);//numofBoids - 1
      float distance = dist(a.location.x, a.location.y, b.location.x, b.location.y);
       if (distance < nodeDistance && i != j){
